...
using AutoMapper;
using Mervi.API.Infrastructure.Swagger;
...
namespace Mervi.API
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            var connectionString = @"Server=.\sqlexpress;Database=Mervi;Integrated Security=True;";
            services.AddDbContext<MerviContext>(o =>
                o.UseSqlServer(connectionString, x => x.MigrationsAssembly("Mervi.Database.Migrations")));
        ...
        private void SwaggerGen(SwaggerGenOptions options)
        {
            _versions.ForEach(version => options.SwaggerDoc(version,
                new Info { Title = "Mervi", Version = $"v{version}" }));
